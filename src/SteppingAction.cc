...
{ 
  ...
  if(aStep->GetTrack()->GetParentID() == 0){
    ...
    man->FillNtupleDColumn(1,4,aStep->GetTrack()->GetPosition().x()/cm);
    man->FillNtupleDColumn(1,5,aStep->GetTrack()->GetPosition().y()/cm);
    man->FillNtupleDColumn(1,6,aStep->GetTrack()->GetPosition().z()/cm);
  ...
  if(aStep->GetTrack()->GetDefinition()->GetParticleName() == "neutron" && aStep->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName() == "nCapture"){
    capx = aStep->GetPostStepPoint()->GetPosition().x();    
    capy = aStep->GetPostStepPoint()->GetPosition().y();
    capz = aStep->GetPostStepPoint()->GetPosition().z();
  }
  


  if(aStep->GetTrack()->GetDefinition()->GetParticleName() =="e-" && aStep->IsFirstStepInVolume() && aStep->GetPreStepPoint()->GetPhysicalVolume()->GetLogicalVolume() == fDetector->fLogicCryostat){
    
    man->FillNtupleDColumn(3,0,capx/cm);
    man->FillNtupleDColumn(3,1,capy/cm);
    man->FillNtupleDColumn(3,2,capz/cm);
    man->FillNtupleDColumn(3,3,aStep->GetTrack()->GetPosition().x()/cm);
    man->FillNtupleDColumn(3,4,aStep->GetTrack()->GetPosition().y()/cm);
    man->FillNtupleDColumn(3,5,aStep->GetTrack()->GetPosition().z()/cm);
    man->AddNtupleRow(3);

  }
  
  if(aStep->GetTrack()->GetDefinition()->GetParticleName() == "opticalphoton" && aStep->IsFirstStepInVolume() && aStep->GetPreStepPoint()->GetPhysicalVolume()->GetLogicalVolume() == fDetector->fLogicCryostat){
    G4double hc = 1.23984*100*eV*nm; //ev/um
    G4double photonE = aStep->GetPreStepPoint()->GetKineticEnergy()/eV;
    G4double lambda = hc/photonE/nm; //in nm
    //std::cout << "found optical photon with wavelength: " << lambda << std::endl;
    G4int op_wl = man->GetH1Id("op_wl");

    man->FillH1(op_wl, lambda);

    man->FillNtupleDColumn(2,0,capx/cm);
    man->FillNtupleDColumn(2,1,capy/cm);
    man->FillNtupleDColumn(2,2,capz/cm);
    man->FillNtupleDColumn(2,3,aStep->GetTrack()->GetPosition().x()/cm);
    man->FillNtupleDColumn(2,4,aStep->GetTrack()->GetPosition().y()/cm);
    man->FillNtupleDColumn(2,5,aStep->GetTrack()->GetPosition().z()/cm);
    man->AddNtupleRow(2);
